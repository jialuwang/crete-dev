...
static ssize_t e1000_receive(void *nc, const uint8_t *buf, size_t size) {
    if(isKleeExternal) {
...
static ssize_t
e1000_bc_receive(NetClientState *nc, const uint8_t *buf, size_t size)
{
...
// Jialu
static void e1000_bc_mmio_write(void *opaque, hwaddr addr, uint64     _t val, unsigned size);
static uint64_t e1000_bc_mmio_read(void *opaque, hwaddr addr, uns     igned size);

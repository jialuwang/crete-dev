...
e1000_send_packet(E1000State *s, const uint8_t *buf, int size)
{
    ...
    if (s->phy_reg[PHY_CTRL] & MII_CR_LOOPBACK) {
        ...
        // Jialu: placemark to update regs after receive
        RECEIVE_REGS rec_regs;
        klee_update_receive_regs(&rec_regs);
    	s->mac_reg[RDH] = rec_regs.rdh;
    	s->mac_reg[GPRC] = rec_regs.gprc;
    	s->mac_reg[TPR] = rec_regs.tpr;
    	s->mac_reg[TORL] = rec_regs.torl;
    	s->mac_reg[TORH] = rec_regs.torh;
    	s->mac_reg[STATUS] = rec_regs.status;
...
static ssize_t e1000_receive(NetClientState *nc, const uint8_t *buf, size_t size) {
    if(isKleeExternal) {
        ...
        E1000State *s = qemu_get_nic_opaque(nc);
        ssize_t ret;
        //ret = e1000_bc_receive(nc, buf, size);
        return ret;
    } else {

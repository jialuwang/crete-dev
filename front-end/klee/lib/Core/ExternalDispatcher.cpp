...
bool ExternalDispatcher::executeCall(Function *f, Instruction *i, uint64_t *args) {
  ...
  if (it == dispatchers.end()) {
	  //fprintf(stderr, "externalDispatcher:executeCall branch 1\n");
#ifdef WINDOWS
    ...
    if (dispatcher) {
      ...
      // function.
  	  //fprintf(stderr, "externalDispatcher:executeCall branch 2\n");

      executionEngine->recompileAndRelinkFunction(dispatcher);
    }
  } else {
	  //fprintf(stderr, "externalDispatcher:executeCall branch 3\n");
...
bool ExternalDispatcher::runProtectedCall(Function *f, uint64_t *args) {
  ...
  if (!f)
	  //fprintf(stderr, "externalDispatcher:runProtectedCall branch 1\n");
  ...
  if (setjmp(escapeCallJmpBuf)) {
	  //fprintf(stderr, "externalDispatcher:runProtectedCall branch 2\n");

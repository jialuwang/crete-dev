...
private:
  ...
  std::set<std::string> labeledTransactions;
  ...
  MemoryObject * handleMemoryMissing(ExecutionState &state, uint64_t addr,
//          void *addr,
          unsigned size,
          bool isReadOnly /* undef if read */);
...
public:
  ...
  virtual void executorInit(llvm::Function *f,
							int argc,
							char **argv,
							char **envp);

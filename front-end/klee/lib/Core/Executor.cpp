...
void Executor::terminateState(ExecutionState &state) {
  ...
  unsigned size2 = labeledTransactions.size();
  for(unsigned i = 0; i < state.coveredInsts.size(); ++i) {
    ss << state.coveredInsts[i].first << state.coveredInsts[i].second;
  }
  transactions.insert(ss.str());
  if(size != transactions.size()) {
	  // A new transaction is found
	  // Write to the file
  }
  ss << "----" << state.requestAddress;
  labeledTransactions.insert(ss.str());
  if(size2 != labeledTransactions.size()) {
	  // A new labeled transaction is found
	  // Write to the file
...
//    klee_warning("Klee call external function qklee_isKleeExternal %s", isKleeExternal?"true":"false");
  ...
  //fprintf(stderr, "KDBG: Calling external function %s\n", function->getName().str().c_str());
...
//Qin: dump external call return value
  ...
  //fprintf(stderr, "KDBG: external call return!\n");
}
...
void Executor::executeMemoryOperation(ExecutionState &state,
		  ...
		  if (isWrite) {
			  if (os->readOnly) {
				  ...
				  if (mo->isExternal) {
					  ...
					  std::string ad;
					  cast<ConstantExpr>(address)->toString(ad);
					  //fprintf(stderr, "address = %s width = %d\n", ad.c_str(), address->getWidth());
					  uint64_t addr = cast<ConstantExpr>(address)->getAPValue().getZExtValue();
					  //uint64_t addr = cast<ConstantExpr>(address)->getZExtValue(address->getWidth());
					  ConstantExpr *CE = cast<ConstantExpr>(value);
					  assert(CE);
					  int bytes = CE->getWidth() / 8;
					  //fprintf(stderr,"bytes = %d\n", bytes);
					  for (int i = 0; i < bytes; i++) {
						  unsigned kw = value->getWidth();
						  //assert(w > 0 && off + w <= kw && "invalid extract");
						  //fprintf(stderr, "off = i * 8 = %d, kw = %d\n", i * 8, kw);
						  ref<Expr> exp_byte = ExtractExpr::create(value, i * 8, Expr::Int8);
						  //fprintf(stderr, "addr = %lx\n", (uint8_t *) addr + i);
						  uint8_t val_byte = cast<ConstantExpr>(exp_byte)->getZExtValue();
						  * ((uint8_t *) addr + i) = (uint8_t)val_byte;
					  }
				  }
			  }
		  } else {
  ...
  if (unbound) {
	  if (incomplete) {
		  ...
		  //Qin
		  handleMemoryMissing(state, cast<ConstantExpr>(address)->getZExtValue(), bytes, false);
...
static void  cross_check_mo_qemu_klee(const ExecutionState& state,
        const MemoryObject *mo, uint64_t offset, uint64_t size)
{
    uint64_t mo_addr = mo->address;
    uint64_t mo_size = mo->size;

    assert((mo_addr + mo_size) >= (mo_addr + offset + size));

    const ObjectState *os = state.addressSpace.findObject(mo);
    assert(os &&
            "[CRETE ERROR] the corresponding os for a given mo is not found.\n");

    for(uint64_t i = 0; i < size; ++i)
    {
        uint8_t byte_value_klee = llvm::cast<klee::ConstantExpr>(os->read8(offset + i))->getZExtValue();
        uint8_t byte_value_qemu = *((uint8_t *)(mo_addr + offset + i));

        assert(byte_value_klee == byte_value_qemu);
    }
}

/* Merge the current existed MOs with the given MO.
 * mo_start_addr: the start address of the given MO
 * mo_size: the size of the given MO
 * return the merged MO
 * Bytes that are not allocated within KLEE's virtual address space
 * will be assigned by the value from the same address within klee/qemu's
 * address space. Existing bytes will be checked against the value from
 * klee/qemu's address space.
 * */

MemoryObject *Executor::crete_merge_overlapped_mos(ExecutionState &state,
        uint64_t mo_start_addr, uint64_t mo_size, bool isReadOnly){
    ...
    if(overlapped_mos.front()->address <= mo_start_addr &&
        ...
        cross_check_mo_qemu_klee(state, overlapped_mos.front(), 0, overlapped_mos.front()->size);

        return overlapped_mos.front();
    }

    // Put overlapped MOs into a sorted map
    ...
    for(std::map<uint64_t, MemoryObject*>::iterator it = ordered_overlapped_mos.begin();
        ...
        if(temp_mo->address != previous_mo_end_addr) {
            // Make up the bytes that are not allocated in the overlapped mos
            ...
            for(uint64_t i = 0; i < makeup_size; ++i){
                uint8_t byte_value_qemu = *((uint8_t *)(previous_mo_end_addr + i));
                old_os_value.push_back(ConstantExpr::create(byte_value_qemu, 8));
            }
        }

        previous_mo_end_addr = temp_mo->address + temp_mo->size;

        //1. save the original value of the existing MOs;
    ...
    MemoryObject *new_mo = memory->allocateFixed(new_mo_start_addr, new_mo_size, 0);
    ...
    //6. perform cross check
    cross_check_mo_qemu_klee(state, new_mo, 0, new_mo->size);
...
//Qin
MemoryObject * Executor::handleMemoryMissing(ExecutionState &state, uint64_t addr,
		//void *addr,
		unsigned size, 
		bool isReadOnly) {
	//klee_message("enter handleMemoryMissing\n");
	const char* fname = state.prevPC->inst->getParent()->getParent()->getName().data();
//	fprintf(stderr, "function name is %s\n", fname);

//	fprintf(stderr, "###### AllocateFixed state* = %p Addr = %lx of size %d\n", &state, addr, size);
	...
	if( memory->isOverlappedMO(addr, size) ){
	    ...
	    mo = memory->allocateFixed((uint64_t) (unsigned long) addr, size, 0);
...
void Executor::handleMemoryMissing(ExecutionState &state,
    ...
    for(uint64_t i = 0; i < bytes; ++i){
        //CHENBO: xxx a check before dereference is needed
...
void Executor::executorInit(Function *f,
				 int argc,
				 char **argv,
				 char **envp) {
...
//void Executor::executorRun(int enableQKLEE, unsigned long requestAddress) {
  fprintf(stderr, "enter executorRun\n");
...
void Executor::executorExit(void) {
  ...
  llvm::errs() << "The number of transactions is " << transactions.size() << "\n";
}

...
static KleeHandler *theHandler = 0;

static time_t t[2];

#if LLVM_VERSION_CODE < LLVM_VERSION(3, 5)
static OwningPtr<MemoryBuffer> BufferPtr;
#endif
...
int qklee_main(int argc, char **argv, char **envp) {  
  ...
  KleeHandler *handler = theHandler = new KleeHandler(pArgc, pArgv);
  ...
  if (!ReplayOutDir.empty() || !ReplayOutFile.empty()) {
    ...
    for (std::vector<KTest*>::iterator
      ...
      interpreter->executorInit(mainFn, out->numArgs, out->args, pEnvp);
    ...
    interpreter->executorInit(mainFn, pArgc, pArgv, pEnvp);
...
int qklee_entry(int qargc, char **qargv, char **qenvp) {
    int argc;
    char **argv;
    char **envp;
    char fname[] = "/home/jialuwang/crete/crete-dev/front-end/model/svd_main.o";
...
int qklee_exit(void) {
  theInterpreter->executorExit();

  char buf[256];
  llvm::raw_ostream &infoFile = theHandler->getInfoStream();
  ...
  stats << "KLEE: done: completed paths = " 
        << theHandler->getNumPathsExplored() << "\n";
  stats << "KLEE: done: generated tests = " 
        << theHandler->getNumTestCases() << "\n";
  ...
  theHandler->getInfoStream() << stats.str();
...
void qklee_request_cpy(void *dst, unsigned size) {
  memcpy(dst, (void *)&mmio_request, sizeof(MMIO_REQUEST));
}
...
*/
/*void qklee_sendMessage(int s) {
    char value = 'a';
    sendMessage(s, &value, 1);
}

void qklee_recvMessage(FILE *fp) {
    uint32_t buf;
    recvMessage(fp, (char *) &buf);
}
*/

// uncomment next four functions when qfm folder presents
...
/*void qklee_sendMessage(int s, void* message, int len) {
  ...
  int i = len;
  for(i = len; i > 0; i--){
    sendMessage(s, (char*)message + i - 1, 1);
  }
...
void qklee_getIndex(int s, FILE*fp, void* message, void* buf, int len){
    ...
    for(i=len; i>0; i--){
         sendMessage(s, (char*)message+i-1, 1);
         fprintf(stderr, "====qklee_getIndex SEND %X\n", *((char*)message+i-1));
         recvMessage(fp, (char *)&t);
         *((char*)buf+i-1) = t;
         fprintf(stderr, "====qklee_getIndex RECV %X\n", *((char *)buf+i-1));
    }
}
...
void qklee_getVal(int s, FILE*fp, void* message, void* buf, int len){
    ...
    for(i=len; i>0; i--){
         sendMessage(s, (char*)message+i-1, 1);
         fprintf(stderr, "====qklee_getVal SEND %X\n", *((char*)message+i-1));
         recvMessage(fp, (char *)&t);
         *((char*)buf+i-1) = t;
         fprintf(stderr, "====qklee_getVal RECV %X\n", *((char *)buf+i-1));
    }
}*/
